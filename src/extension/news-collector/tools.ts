...
export async function globNews(
  ...
  options: { pattern: string; metadataFilter?: Record<string, string>; limit?: number },
...
export async function grepNews(
  context: NewsToolContext,
  options: { pattern: string; contextChars?: number; metadataFilter?: Record<string, string>; limit?: number },
...
export function createNewsArchiveTools(provider: INewsProvider) {
  return {
    globNews: tool({
      description: `Search collected news archive by title pattern. Returns matching headlines with index, title, content length, and metadata. Use lookback: "1h", "12h", "1d", "7d" to control time range.
...
Example: globNews({ pattern: "BTC|Bitcoin", lookback: "1d" })`,
    ...
    grepNews: tool({
      description: `Search collected news content by pattern (like grep). Returns matched text with surrounding context.
...
Example: grepNews({ pattern: "interest rate", lookback: "2d" })`,
    ...
    readNews: tool({
      description: `Read full content of a collected news item by index. Use after globNews/grepNews. Use same lookback for consistent indices.`,
      inputSchema: z.object({
